...
public class LegendaryPotionItem extends Item {
    private static final Random RANDOM = new Random();
    private static final float LEGENDARY_SPAWN_CHANCE = 0.5f; // 0.5% chance

    public LegendaryPotionItem(Item.Properties properties) {
        super(properties);
    }
            ...
            if (success) {
                ...
                if (server != null) {
                    ...
                    CustomConsumables.getLogger().info(
                            "Player {} used Legendary Potion successfully! Spawning a legendary.",
    ...
    public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent(TextFormatting.GOLD + "Legendary Potion"));
        tooltip.add(new StringTextComponent(TextFormatting.YELLOW + "Has a " +
                TextFormatting.GREEN + LEGENDARY_SPAWN_CHANCE + "%" +
                TextFormatting.YELLOW + " chance to spawn a legendary Pokémon when consumed"));
